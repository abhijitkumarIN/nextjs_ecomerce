const CheckboxColor = ({ color, name, type, onChange, valueName, checked }) => {
  const onSelect = (e) => {
  ...
  return (
    <label htmlFor={color+'-'+name} className={`checkbox-color`}>
      <input onChange={onSelect} value={color} data-name={valueName} name={name} type={type} id={color + '-' + name} checked={checked}/>
